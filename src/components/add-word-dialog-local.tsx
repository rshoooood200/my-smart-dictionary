...
export function AddWordDialog({ open, onOpenChange }: AddWordDialogProps) {
  ...
  // 3 جمل مع الترجمة العربية
  const [sentence1, setSentence1] = useState<SentencePair>({ en: '', ar: '' })
  const [sentence2, setSentence2] = useState<SentencePair>({ en: '', ar: '' })
  const [sentence3, setSentence3] = useState<SentencePair>({ en: '', ar: '' })
  ...
  const handleGenerateInfo = async () => {
    ...
    try {
      ...
      // ✅ إصلاح: 3 جمل مع الترجمة العربية - استخراج الجملة والترجمة معاً
      if (data.sentences && data.sentences.length > 0) {
        setSentence1({
          en: data.sentences[0]?.sentence || '',
          ar: data.sentences[0]?.translation || '',
        })
        setSentence2({
          en: data.sentences[1]?.sentence || '',
          ar: data.sentences[1]?.translation || '',
        })
        setSentence3({
          en: data.sentences[2]?.sentence || '',
          ar: data.sentences[2]?.translation || '',
        })
      } else if (data.examples && data.examples.length > 0) {
        // fallback للبيانات القديمة بدون ترجمة عربية
        setSentence1({ en: data.examples[0] || '', ar: '' })
        setSentence2({ en: data.examples[1] || '', ar: '' })
        setSentence3({ en: data.examples[2] || '', ar: '' })
      }
  ...
  const handleSubmit = async () => {
    ...
    // ✅ إصلاح: بناء مصفوفة الجمل مع الترجمة العربية
    const sentences: { sentence: string; translation: string }[] = []
    if (sentence1.en.trim()) {
      sentences.push({ sentence: sentence1.en.trim(), translation: sentence1.ar.trim() })
    }
    if (sentence2.en.trim()) {
      sentences.push({ sentence: sentence2.en.trim(), translation: sentence2.ar.trim() })
    }
    if (sentence3.en.trim()) {
      sentences.push({ sentence: sentence3.en.trim(), translation: sentence3.ar.trim() })
    }

    // أمثلة بسيطة (للتوافق مع الحقل القديم)
    const examples: string[] = []
    if (sentence1.en.trim()) examples.push(sentence1.en.trim())
